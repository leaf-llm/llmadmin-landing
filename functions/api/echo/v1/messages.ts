...
async function handlePOST(request: Request): Promise<Response> {
  ...
  if (system) contentParts.push(system);
  for (const msg of messages) {
    contentParts.push(typeof msg.content === "string" ? msg.content : JSON.stringify(msg.content));
  }

  const response = {
    id: "msg_echo_" + crypto.randomUUID().slice(0, 8),
    type: "message",
    role: "assistant",
    model,
    content: [
      {
        type: "text",
        text: truncate(contentParts.join("\n\n")),
      },
    ],
